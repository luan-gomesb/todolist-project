...
describe.skip("Testing mongo db operations", () => {
	...
	const repository = new TodoRepositoryMongoDB("localhost");
	test.skip("should insert a item", async () => {
		const item: Item = {
	...
	test.skip("find todolist filterer", async () => {
	...
	test(" get todoItem by id", async () => {
		...
		const findInvaidId = await repository.findById("id invalido");
		expect(findInvaidId).toBeNull();
	});
